  ...
  for (const name of _.keys(mods)) {
    ...
    const runner = function () {
      if (item.started > 0) {
        ...
        */
        fastify.log.warn(`[${options.prefix}:${item.name}] still running, ignore execution`)
        return
      }
      item.started = Date.now()
      fastify.log.debug(`[${options.prefix}:${item.name}] started`)
      Promise.resolve()
        .then(() => {
          return item.handler()
        })
        .then(result => {
          fastify.log.debug(`[${options.prefix}:${item.name}] completed in ${Date.now() - item.started} ms`)
          item.started = 0
        })
        .catch(err => {
          fastify.log.error(`[${options.prefix}:${item.name}] ${err.message}`)
          item.started = 0
        })
    }
